...
import { EnrichedProperty, Floor, Building } from '../types';
import {
  RotateCcw,
  Box,
  Compass,
...
export const ThreeCanvas: React.FC<ThreeCanvasProps> = ({
  ...
  useEffect(() => {
    ...
    if (currentBuilding?.model_url && currentBuilding.model_type === 'uploaded_glb') {
    ...
    const bldEnvelopeMat = new THREE.MeshStandardMaterial({
      color: 0xe2e8f0,
      transparent: true,
      opacity: viewMode === 'wireframe' ? 0.04 : 0.12,
      roughness: 0.5,
      metalness: 0.1,
      side: THREE.BackSide,
    });
    ...
    // 2. Floor Plates for all floors (Floor 0: 0-3m, Floor 1: 3-6m, Floor 2: 6-9m, Floor 3: 9-12m)
    ...
    floorsToRender.forEach((fl) => {
      ...
      const slabGeo = new THREE.BoxGeometry(buildingWidth - 0.3, 0.22, buildingLength - 0.3);
      const slabMat = new THREE.MeshStandardMaterial({
        color: isCurrentUnitFloor ? 0x0284c7 : fl.floor_number === 0 ? 0xcfd8dc : 0xe2e8f0,
        transparent: true,
        opacity: isCurrentUnitFloor ? 0.6 : 0.35,
        roughness: 0.6,
      });
      const slabMesh = new THREE.Mesh(slabGeo, slabMat);
      slabMesh.position.set(0, fl.bottom_height + 0.11 + verticalShift, 0);
      ...
      slabLine.position.set(0, fl.bottom_height + 0.11 + verticalShift, 0);
    ...
    // If allProperties is supplied, render all 8 units. Otherwise fallback to single enrichedProperty
    ...
    propertiesToRender.forEach((item) => {
      ...
      // Distinct Color & Material Palettes:
      // 1. Selected Unit: Glowing Vibrant Cyan / Neon Gold
      // 2. Parking (Ground Floor): Slate / Cyan demarcation with parking slots
      // 3. Watchman (Ground Floor): Emerald / Teal security post
      // 4. Residential Flats: Cool Tech Blue / Indigo glass
      let baseColor = 0x2563eb;
      let emissiveColor = 0x1e3a8a;
      let emissiveInt = 0.2;
      ...
      if (isThisSelected) {
        ...
        emissiveColor = flNumber === 1 ? 0x1d4ed8 : flNumber === 2 ? 0x4338ca : 0x6d28d9;
      }
    ...
    // 4. Vertical Height Ruler & Elevation Calipers (Z=0m, 3m, 6m, 9m, 12m)
    if (showRuler) {
      ...
      // Height ticks every 1 meter and major markers for Floor boundaries
      for (let h = 0; h <= totalHeight; h += 1) {
        ...
        const tickLine = new THREE.Line(
          tickGeo,
          new THREE.LineBasicMaterial({
            color: isFloorBoundary ? 0x22d3ee : 0x64748b,
    ...
    <div className="relative w-full h-full min-h-[440px] rounded-xl overflow-hidden bg-slate-100 border border-slate-200 shadow-md flex flex-col">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-[11px] text-slate-600 font-mono">
                  {unmappedCoordinates ? `${unmappedCoordinates.lat.toFixed(5)}, ${unmappedCoordinates.lng.toFixed(5)}` : 'Searched Coordinates'}
                </div>
              </div>
            </div>
      ...
      <div className="absolute bottom-3 left-3 right-3 flex flex-wrap items-center justify-between gap-2 p-2 rounded-lg bg-white/95 border border-slate-200 backdrop-blur-md z-10 shadow-md text-slate-800">
          ...
          {onExplodedOffsetChange && !isInitialBlank && !isEmptyParcel && (
            <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 px-2.5 py-1 rounded-md">
              ...
              <input
                ...
                title={`Explode floors: ${(explodedOffset * 100).toFixed(0)}%`}
              />
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-slate-500 font-mono hidden md:inline">Prototype ID:</span>
          <span className="px-2.5 py-1 text-xs font-mono font-bold rounded bg-cyan-50 text-cyan-800 border border-cyan-200">
